...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    const checkLackOfAvailability = await Appointment.findOne({
      where: {
        provider_id,
        canceled_at: null,
        date: hourStart,
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error: 'You are able to cancel an appointment no later than 2 hours before its beginning.',
      });
    }
